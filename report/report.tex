...
計測に用いたコンピュータの諸元を表\ref{tab:pc}に示す。
...
図\ref{fig:bubrev}を見るとバブルソート、挿入ソートは順当に遅くなっているが選択ソートはほとんど時間が変わっていない。これは選択ソートは毎回の走査でソートされていない部分の全ての要素を見なければならず、配列が逆になっていようが大して変わらないからだと思われる。
...
そして今考えたら割合はrationじゃなくてratioだなって
